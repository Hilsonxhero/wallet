        ...
        <base-skeleton animated :loading="loading">
            ...
            <template #default>
                <div class="flex justify-between mb-12 items-center">
                    <h2 class="text-xl">لیست کیف پول ها</h2>
                    <v-btn
                        :to="{ name: 'management-wallets-create' }"
                        color="blue-accent-2"
                    >
                        ایجاد کیف پول
                    </v-btn>
...
const fetchData = async () => {
    const { data } = await ApiService.get("management/wallets");
...
const handleDelete = async () => {
    const { data } = await ApiService.delete(
        `/api/panel/wallets/${selected_item.value.id}`
    );
    if (data.status == 200) {
